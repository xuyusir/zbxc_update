...
import com.jy.common.utils.PutObjectProgressListener;
...
public class FileServiceImpl extends BaseService implements FileService {
    ...
    public Result insertFile(AccountInfo accountInfo, Integer type, MultipartFile uploadFile, Long directory_id, Long video_time) throws Exception {
        ...
        if (type == 2){
            ...
            fileFullPath = aliyunOSSClientUtil.uploadFile(folder, fileName, uploadFile.getSize(), inputStream, accountInfo.getAccount().getAccountid());
        }
    ...
    @Override
    public Result getFileList(AccountInfo accountInfo, Integer type, String month, Long directory_id, String search, Integer page, Integer pageSize) throws Exception {
    ...
    @Override
    public Result insertVideo(AccountInfo accountInfo, String videoID, String videoName, Long videoTime) throws Exception {

        //获取视频信息
        GetVideoInfoResponse videoInfo = aliyunOSSClientUtil.getVideoInfo(videoID);
        GetVideoInfoResponse.Video video = videoInfo.getVideo();

        Fileinfo fileinfo = new Fileinfo();
        // type 0是图片,1是文件,2是视频,3是面经题库,4是二维码
        fileinfo.setType(2);
        fileinfo.setFilename(videoName);
        fileinfo.setFiletype(videoName.substring(videoName.lastIndexOf(".") + 1));
        fileinfo.setFilepath(getFolder(video.getCateId(), new Date()));
        fileinfo.setFilefullpath(videoID);
        fileinfo.setFilesize(video.getSize() +"");
        fileinfo.setVideotime(videoTime);
        fileinfo.setDocumentdirectoryid(video.getCateId());
        fileinfo.setCreateby(accountInfo.getAccount().getName());
        fileinfo.setCreatepersonid(accountInfo.getAccount().getAccountid());
        fileinfo.setCreatetime(new Date());
        try {
            ...
            //数据库操作日志
            this.dbLog(accountInfo.getAccount().getAccountid(), accountInfo.getAccount().getName(), "上传文件:[ " + videoName + " ]", DB_LOG_TYPE);
        } catch (Exception e) {
            //上传出现异常,删除文件
            aliyunOSSClientUtil.deleteFile(videoID);
            throw new MyException(ResultEnum.UPLOAD_FILE_ERROR);
        }
        fileinfo.setFilepath(aliyunOSSClientUtil.getUrl(videoID));
        //关闭阿里云OSS连接
        aliyunOSSClientUtil.closeOSSClient();
        return Result.succee(fileinfo);
    ...
    @Override
    public Result getProgressBar(AccountInfo accountInfo) {

        Long accountID = accountInfo.getAccount().getAccountid();
        String status = jedisClient.get(PutObjectProgressListener.PROGRESS_BAR_STATUS + accountID, REDIS_PASSWORD);
        String total = jedisClient.get(PutObjectProgressListener.PROGRESS_BAR_TOTAL + accountID, REDIS_PASSWORD);
        String percent = jedisClient.get(PutObjectProgressListener.PROGRESS_BAR_PERCENT + accountID, REDIS_PASSWORD);
        String uploadSize = jedisClient.get(PutObjectProgressListener.PROGRESS_BAR_UPLOAD_SIZE + accountID, REDIS_PASSWORD);
        String succeed = jedisClient.get(PutObjectProgressListener.PROGRESS_BAR_SUCCEED + accountID, REDIS_PASSWORD);

        if (StringUtils.isNotBlank(succeed)){
            jedisClient.del(PutObjectProgressListener.PROGRESS_BAR_STATUS + accountID, REDIS_PASSWORD);
            jedisClient.del(PutObjectProgressListener.PROGRESS_BAR_TOTAL + accountID, REDIS_PASSWORD);
            jedisClient.del(PutObjectProgressListener.PROGRESS_BAR_PERCENT + accountID, REDIS_PASSWORD);
            jedisClient.del(PutObjectProgressListener.PROGRESS_BAR_UPLOAD_SIZE + accountID, REDIS_PASSWORD);
            jedisClient.del(PutObjectProgressListener.PROGRESS_BAR_SUCCEED + accountID, REDIS_PASSWORD);
        }

...
public class BusinessServiceImpl extends BaseService implements BusinessService {
    ...
    public Result insertBusiness(AccountInfo accountInfo, String AUTHORITY_CODE, Business business) throws MyException {
        ...
        if (parent_business== null){
            //行业表中不存在数据,添加所有行业
            Business parentBusiness = new Business();
            parentBusiness.setBusinessid(business.getParentid());
            parentBusiness.setBusinessname("所有行业");
            parentBusiness.setParentid(0);
            parentBusiness.setIsparent(true);
            parentBusiness.setStatus(1);
            parentBusiness.setCreatetime(new Date());
            parentBusiness.setCreateby("系统超级管理员");
            parentBusiness.setCreatepersonid(1L);
            int i = businessMapper.insertSelective(parentBusiness);
            if (i < 1){
                throw new MyException(ResultEnum.INSERT_SQL_ERROR);
            }
        }else if (!parent_business.getIsparent()) {
            //判断添加前父节点是否是父节点,不是父节点修改为父节点
            Business business2 = new Business();
            business2.setBusinessid(parent_business.getBusinessid());
            business2.setIsparent(true);
            int i = businessMapper.updateByPrimaryKeySelective(business2);
            if (i < 1) {

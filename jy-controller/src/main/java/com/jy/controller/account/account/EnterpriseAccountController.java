...
public class EnterpriseAccountController extends BaseController {
	...
	public Result getEnterpriseAccountList(
		...
		return accountService.getEnterpriseAccountList(isDelete, search, page, pageSize);
	}
	...
	public Result exportEnterpriseAccountList(
			@ApiParam(value = "关键字,模糊查询企业名称,联系人,联系电话", required = false) @RequestParam(required = false, defaultValue = "") String search,
		...
		Result result = accountService.exportEnterpriseAccountList(false, accountInfo, AuthorityCode.ACCOUNT_ENTERPRISE_ACCOUNT_EXPORT_AUTHORITY, search);
		if (result.getStatus() != 200) {
			...
			String fileName = "";
			response.setHeader("Content-disposition", "attachment; filename=" + new String("中博星辰企业账号列表.xls".getBytes("utf-8"), "ISO8859-1"));
	...
	@ApiOperation(value = "回收企业账号接口", httpMethod = "PUT", produces = "application/json")
	...
	public Result recycleEnterpriseAccount(
		...
		AccountInfo accountInfo = this.getAccountInfo();
		return accountService.recycleAccount(accountInfo, AuthorityCode.ACCOUNT_ENTERPRISE_ACCOUNT_RECYCLE_AUTHORITY, ids, 3);
	}

	/*@ApiOperation(value = "彻底删删除企业账号", httpMethod = "PUT", produces = "application/json")
	@RequestMapping(value = "final/delete", method = RequestMethod.PUT)
	@ResponseBody
	public Result finalDeleteEnterpriseAccount(
			@ApiParam(value = "要测地删除的账号ID列表,用\",\"号隔开", required = true) @RequestParam(required = true) String ids
	) throws MyException {
		AccountInfo accountInfo = this.getAccountInfo();
		return accountService.finalDeleteEnterpriseAccount(accountInfo, AuthorityCode.ACCOUNT_ENTERPRISE_ACCOUNT_FINAL_DELETE_AUTHORITY, ids);
	}*/
